...
- [x] タスク1〜8（テスト導入・小バグ修正・普及台数減衰・セーブバージョニング・副作用分離）— 2026-08-23 完了
- [x] タスク9（game.svelte.ts 分割: 7モジュール＋バレル）— 2026-08-23 完了（`029cb75`）
- [x] タスク10（App.svelte 分割: 12コンポーネント、1077行→383行）— 2026-08-23 完了（`93ea27b`）
- [x] タスク11（biome 導入）— 2026-08-23 完了（`f491da4`）
- [x] タスク12（オフィス規模・移転・社員上限・家賃・ストレス・休憩室アップグレード）— 2026-09-02 完了（`1e9a94d` / `e58d758` / `7cb1b04`）
- [ ] 要人間レビューの機能系タスク（借金・競合データ活用・ランキング等）は未着手
...
| ストレス・雇用上限 | §3.1 / §3.8 | ストレス0〜100・休憩室、オフィス規模による社員数上限 → タスク12で実装済み（`1e9a94d` / `e58d758` / `7cb1b04`） |
...
| オフィス移転・設計 | §3.8 / §4.4 | 小→中→大規模移転、机配置で効率変化 → 移転はタスク12で実装済み、机・レイアウト（増設・混雑度・方針3択）も実装済み |
