...
- テスト（`tests/simulation.test.ts`・全18テスト）: 40年プレイに移転・雇用・休憩室を組み込み、DLC開始の空きスタジオ探索バグを修正（40年でDLC 38本を検証）。オフィス移転条件・社員上限・家賃・ストレス・休憩室改修・机増設・混雑度・レイアウト・スカウト手段・売上予測・出荷単位・ハード牽引の単体検証を追加
