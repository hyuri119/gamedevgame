...
- 出荷レベル制（`src/lib/sales.svelte.ts`）: 上限=1万本×出荷Lv（初期Lv1）・月1回レベルアップ（100万円×Lv）・大型生産力増強は100倍費用で+10Lv何度でも可・再出荷は月1本のみ。テナント大型生産力開発所は上限2倍に変更。自動テストは需要に見合った投資（10年は上限10万・40年は30万まで月次上げ）で健全性維持
- 初期社員: プレイヤー自身「あなた」（ディレクターLv3・契約金0・年俸600万円・解雇不可）が1枠目に在籍。旧セーブは読込時に補完
- 駆け引き要素: スカウトは4手段（ハローワーク無料〜ヘッドハンティング2000万円。費用が高いほど候補層が精鋭に。`SCOUT_METHODS`）/ 売上予測は真値の±15%・1000本単位表示で実需と分離（`forecastSalesOf`・`makeForecast`。旧セーブは真値にフォールバック）/ 出荷・再出荷は1000本単位に切り捨て（入力はstep=1000・予測分ボタン。ヘッダー進捗欄の即出荷ボタンは廃止し開発詳細に誘導）/ キラータイトル効果（自社ソフト販売本数の1/10がそのハードの普及台数に加算。PC除く。`hwBoost`）
...
- テスト（`tests/simulation.test.ts`・全21テスト）: 40年プレイに移転・雇用・休憩室を組み込み、DLC開始の空きスタジオ探索バグを修正（40年でDLC 38本を検証）。オフィス移転条件・社員上限・家賃・ストレス・休憩室改修・机増設・混雑度・レイアウト・スカウト手段・売上予測・出荷単位・ハード牽引・出荷レベル・再出荷制限・初期社長の単体検証を追加
- 次回候補: 相性データのマスク（内容×ジャンル相性を見せない。試した組み合わせだけ開示する等。`data/kumiawase.csv` は内部データのまま）/ バランス数値の本調整v2（v1で40年482億円→423億円まで圧縮。方針として開発回転率は高め維持・放置ゲーなので難易度は上げすぎない。売上側の調整は慎重に）/ 史実モードとゲームモードの切替（ハード登場密度：史実通り vs 間引き。`doc/siyou.md` §8 の未決定事項として残す）/ 競合データ活用（売上ランキング・発売日被り）
