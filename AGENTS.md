...
- 駆け引き要素: スカウトは調査費200万円/回（`SCOUT_COST`）/ 売上予測は真値の±15%・1000本単位表示で実需と分離（`forecastSalesOf`・`makeForecast`。旧セーブは真値にフォールバック）/ 出荷・再出荷は1000本単位に切り捨て（入力はstep=1000・予測分ボタン）
...
- テスト（`tests/simulation.test.ts`・全16テスト）: 40年プレイに移転・雇用・休憩室を組み込み、DLC開始の空きスタジオ探索バグを修正（40年でDLC 38本を検証）。オフィス移転条件・社員上限・家賃・ストレス・休憩室改修・机増設・混雑度・レイアウトの単体検証を追加
